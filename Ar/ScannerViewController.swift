...
class ScannerViewController: UIViewController, STBackgroundTaskDelegate, MeshViewDelegate, UIGestureRecognizerDelegate, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if !defaults.bool(forKey: "instructionOverlay") {
            ...
            Timer.schedule(delay: 10.0, handler: {_ in
                self.instructionOverlay.isHidden = false
                self.instructionOverlay.alpha = 1
                Timer.schedule(delay: 15.0, handler: { _ in
	...
	func setupUserInterface() {
        ...
        FileMgr.sharedInstance.useSubpath(subPath: "scannerCache")
	...
	func setupIMU() {
		...
		let dmHandler: CMDeviceMotionHandler = { motion, _ in
			...
			if self.view != nil {
                self.processDeviceMotion(motion: motion!, error: nil)
			}
		}
	...
	func adjustVolumeSize(volumeSize volumeSize: GLKVector3) {
        ...
        let x = keepInRange(value: volumeSize.x, minValue: 0.1, maxValue: 3)
        let y = keepInRange(value: volumeSize.y, minValue: 0.1, maxValue: 3)
        let z = keepInRange(value: volumeSize.z, minValue: 0.1, maxValue: 3)
	...
	func updateAppStatusMessage() {
		...
		case .NeedsUserToConnect:
            showAppStatusMessage(msg: _appStatus.pleaseConnectSensorMessage)
			return

		case .NeedsUserToCharge:
            showAppStatusMessage(msg: _appStatus.pleaseChargeSensorMessage)
			return
        ...
        if sender.state == .began {
			...
			if _slamState.scannerState == .CubePlacement {
				...
				if !_volumeScale.initialPinchScale.isNaN {
                    ...
                    _volumeScale.currentScale = CGFloat(keepInRange(value: Float(_volumeScale.currentScale), minValue: 0.01, maxValue: 1000))
