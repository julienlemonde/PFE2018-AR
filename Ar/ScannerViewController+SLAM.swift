...
extension ScannerViewController {
    ...
    func setupSLAM() {
        ...
        let trackerOptions: [NSObject : AnyObject] = [kSTTrackerTypeKey as NSObject: _dynamicOptions.newTrackerIsOn ? STTrackerType.depthAndColorBased.rawValue as AnyObject: STTrackerType.depthBased.rawValue as AnyObject, kSTTrackerTrackAgainstModelKey as NSObject: true as AnyObject, kSTTrackerQualityKey as NSObject: STTrackerQuality.accurate.rawValue as AnyObject, kSTTrackerBackgroundProcessingEnabledKey as NSObject: true as AnyObject]
		...
		let keyframeManagerOptions: [NSObject : AnyObject] = [
            kSTKeyFrameManagerMaxSizeKey as NSObject: _options.maxNumKeyFrames as AnyObject,
			kSTKeyFrameManagerMaxDeltaTranslationKey as NSObject: _options.maxKeyFrameTranslation as AnyObject,
			kSTKeyFrameManagerMaxDeltaRotationKey as NSObject: _options.maxKeyFrameRotation as AnyObject] // 20 degrees.
	...
	func setupMapper() {
        ...
        voxelSizeInMeters = keepInRange(value: voxelSizeInMeters, minValue: 0.003, maxValue: 0.2)
		...
		let mapperOptions: [NSObject: AnyObject] =
            [kSTMapperLegacyKey as NSObject : !_dynamicOptions.newMapperIsOn as AnyObject,
			 kSTMapperVolumeResolutionKey as NSObject: voxelSizeInMeters as AnyObject,
			 kSTMapperVolumeBoundsKey as NSObject: [volumeBounds.x, volumeBounds.y, volumeBounds.z] as AnyObject,
			 kSTMapperVolumeHasSupportPlaneKey as NSObject: _slamState.cameraPoseInitializer!.hasSupportPlane as AnyObject,
			 kSTMapperEnableLiveWireFrameKey as NSObject: false as AnyObject,
			 ]
        ...
        if _slamState.keyFrameManager!.wouldBeNewKeyframe(withColorCameraPose: colorCameraPoseAfterTracking) {
			...
			if isFirstFrame { // always add the first frame.
				...
				if deltaSeconds < Double(frameDuration.value) / Double(frameDuration.timescale) * 2 {
                    ...
                    deltaAngularSpeedInDegreesPerSecond = deltaRotationAngleBetweenPosesInDegrees (previousPose: depthCameraPoseBeforeTracking, newPose: depthCameraPoseAfterTracking()) / Float(deltaSeconds)
				}
    ...
    func processDepthFrame(depthFrame: STDepthFrame, colorFrame: STColorFrame?) {
        ...
        if _useColorCamera && colorFrame != nil {
            ...
            uploadGLColorTextureFromDepth(depthFrame: depthFrame)
        }
        ...
        case .Scanning:
            ...
            do {
                ...
                trackingMessage = computeTrackerMessage(hints: _slamState.tracker!.trackerHints)
				
				// Set the mesh transparency depending on the current accuracy.
                updateMeshAlphaForPoseAccuracy(poseAccuracy: _slamState.tracker!.poseAccuracy)
                ...
                keyframeMessage = maybeAddKeyframeWithDepthFrame(depthFrame: depthFrame, colorFrame: colorFrame, depthCameraPoseBeforeTracking: depthCameraPoseBeforeTracking)
...
extension ScannerViewController: STSensorControllerDelegate {
