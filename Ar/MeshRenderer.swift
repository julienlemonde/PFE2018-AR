...
class MeshRenderer: NSObject {
  ...
  func releaseGLBuffers() {
	...
	deinit {
        ...
        MeshRendererDestructor(d: self.d!)
   ...
   func getRenderingMode() -> RenderingMode {
    ...
    func uploadMesh(mesh: STMesh) {
        ...
        if d!.hasTexture {
            ...
            uploadTexture(pixelBuffer: pixelBuffer())
        }
    ...
    func renderPartialMesh (meshIndex: Int) {
        ...
        case RenderingMode.XRay:
            ...
            enableLinesElementBuffer(meshIndex: meshIndex)
            enableVertexBuffer(meshIndex: meshIndex)
            enableNormalBuffer(meshIndex: meshIndex)
            glDrawElements( GLenum(GL_LINES), GLsizei(d!.numLinesIndices[meshIndex]), GLenum(GL_UNSIGNED_SHORT), nil)
            disableNormalBuffer(meshIndex: meshIndex)
            disableVertexBuffer(meshIndex: meshIndex)

        case RenderingMode.LightedGray:

            enableTrianglesElementBuffer(meshIndex: meshIndex)
            enableVertexBuffer(meshIndex: meshIndex)
            enableNormalBuffer(meshIndex: meshIndex)
        ...
        case RenderingMode.PerVertexColor:
            ...
            enableVertexBuffer(meshIndex: meshIndex)
            enableNormalBuffer(meshIndex: meshIndex)
            enableVertexColorBuffer(meshIndex: meshIndex)
            glDrawElements( GLenum(GL_TRIANGLES), GLsizei(d!.numTriangleIndices[meshIndex]), GLenum(GL_UNSIGNED_SHORT), nil)
            disableVertexColorBuffer(meshIndex: meshIndex)
            disableNormalBuffer(meshIndex: meshIndex)
            disableVertexBuffer(meshIndex: meshIndex)
        ...
        case RenderingMode.Textured:
            ...
            enableTrianglesElementBuffer(meshIndex: meshIndex)
            enableVertexBuffer(meshIndex: meshIndex)
            enableVertexTexcoordsBuffer(meshIndex: meshIndex)
            glDrawElements( GLenum(GL_TRIANGLES), GLsizei(d!.numTriangleIndices[meshIndex]), GLenum(GL_UNSIGNED_SHORT), nil)
            disableVertexTexcoordBuffer(meshIndex: meshIndex)
            disableVertexBuffer(meshIndex: meshIndex)
        }
 ...
 func render(projectionMatrix: UnsafePointer<GLfloat>, modelViewMatrix: UnsafePointer<GLfloat>) {
        ...
        case RenderingMode.XRay:
            d!.xRayShader!.enable()
            d!.xRayShader!.prepareRendering(projection: projectionMatrix, modelView: modelViewMatrix)

        case RenderingMode.LightedGray:
            d!.lightedGrayShader!.enable()
            d!.lightedGrayShader!.prepareRendering(projection: projectionMatrix, modelView: modelViewMatrix)
        ...
        case RenderingMode.PerVertexColor:
            ...
            d!.perVertexColorShader!.prepareRendering(projection: projectionMatrix, modelView: modelViewMatrix)
        ...
        case RenderingMode.Textured:
            ...
            d!.yCbCrTextureShader!.prepareRendering(projection: projectionMatrix, modelView: modelViewMatrix, textureUnit: GLint(d!.textureUnit))
        }
